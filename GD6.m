...
y = test(:,4); % observed data
test(:,4) = []; %separates observed data from input values <- essentially (xi1..xij,yi)

x=test;
x(:,(1:3)) = normalize(test(:,(1:3)),'zscore');%normalizes the input x values

[o,p] = size(test);

bias = 0; % initialise the bias
learningRate =0.7; %fixed learning rate of 0.7
count =0;
MSEUpdate = [];
MSEBoxplot = [];

for v = 1:100
weight= -0.7 + (0.7--0.7).*rand(1,p); % initialised the weights between -0.7 and 0.7 for each iteration   
while (count<1000) %fixed stopping rule of 1000
...
count = 0; %reset count for each overall iteration
MSEBoxplot = [MSEBoxplot MSE];
...
xlabel('Test Set');
title('Mean Squared Error for each iteration of logistic regression program');